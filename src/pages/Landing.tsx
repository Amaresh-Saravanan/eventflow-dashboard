      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-lg border-b border-border">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to="/login" className="h-9 px-4 rounded-lg bg-primary text-primary-foreground text-sm font-medium flex items-center gap-2 hover:opacity-90 transition-opacity">
              Get Started
      ...
      <section className="pt-32 pb-20 px-6">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link to="/login" className="h-12 px-8 rounded-xl bg-primary text-primary-foreground font-medium flex items-center gap-2 hover:opacity-90 transition-opacity">
              Start for free
              ...
              <div className="rounded-xl border border-border/50 overflow-hidden">
                ...
                <div className="bg-[#16142B] p-6 space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    label: "Avg. Latency",
                    value: "45ms"
                  }].map(stat => <div key={stat.label} className="bg-[#1E1B36] rounded-xl p-4 border border-[rgba(255,255,255,0.04)]">
      ...
      <section id="features" className="py-20 px-6 bg-card">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map(feature => <div key={feature.title} className="p-6 rounded-2xl bg-background border border-border hover:border-primary/30 transition-colors">
      ...
      <section id="pricing" className="py-20 px-6">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {pricingPlans.map(plan => <div key={plan.name} className={`relative p-6 rounded-2xl border ${plan.popular ? "border-primary bg-primary/5" : "border-border bg-background"}`}>
                {plan.popular && <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full bg-primary text-primary-foreground text-xs font-medium">
                    Most Popular
                  </div>}
                ...
                <Link to="/login" className={`block w-full py-2.5 rounded-lg text-center text-sm font-medium transition-colors ${plan.popular ? "bg-primary text-primary-foreground hover:opacity-90" : "bg-secondary text-foreground hover:bg-secondary/80"}`}>
                  Get started
                </Link>
              </div>)}
          </div>
      ...
      <section className="py-20 px-6">
        <div className="max-w-4xl mx-auto text-center">
          <div className="p-12 rounded-3xl gradient-accent">
            ...
            <Link to="/login" className="inline-flex h-12 px-8 rounded-xl bg-primary text-primary-foreground font-medium items-center gap-2 hover:opacity-90 transition-opacity">
