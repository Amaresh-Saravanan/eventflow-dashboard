...
const columns = [
  {
    ...
    header: "Event ID",
    render: (item: Event) => (
      <code className="text-xs bg-secondary px-2 py-1 rounded-md font-mono">{item.eventId}</code>
    ),
  },
  {
  ...
  },
  {
    ...
    header: "Event Type",
    render: (item: Event) => (
      <code className="text-xs text-muted-foreground font-mono">{item.eventType}</code>
    ),
  ...
  },
  {
    ...
    header: "Duration",
    render: (item: Event) => (
      <span className={`text-sm tabular-nums ${item.duration.includes('s') && !item.duration.includes('ms') ? 'text-warning' : ''}`}>
        {item.duration}
      </span>
    ),
  ...
  },
  {
    ...
    header: "Timestamp",
    render: (item: Event) => (
      <span className="text-sm text-muted-foreground tabular-nums">{item.timestamp}</span>
    ),
...
const Events = () => {
  ...
  return (
    <div className="animate-fade-in">
      <DashboardHeader 
        title="Events" 
        subtitle="View all webhook events and their delivery status" 
      />
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button className="h-9 px-4 rounded-lg bg-secondary text-sm font-medium hover:bg-secondary/80 transition-colors disabled:opacity-50" disabled>
              Previous
            </button>
            <div className="flex items-center gap-1">
              <button className="w-9 h-9 rounded-lg bg-primary text-primary-foreground text-sm font-medium">1</button>
              <button className="w-9 h-9 rounded-lg bg-secondary text-sm font-medium hover:bg-secondary/80">2</button>
              <button className="w-9 h-9 rounded-lg bg-secondary text-sm font-medium hover:bg-secondary/80">3</button>
              <span className="px-2 text-muted-foreground">...</span>
              <button className="w-9 h-9 rounded-lg bg-secondary text-sm font-medium hover:bg-secondary/80">125</button>
            </div>
            <button className="h-9 px-4 rounded-lg bg-secondary text-sm font-medium hover:bg-secondary/80 transition-colors">
              Next
            </button>
          </div>
        </div>
